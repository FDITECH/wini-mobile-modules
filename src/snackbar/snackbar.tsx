...
//                 return ColorSkin.inforColor
//             case ComponentStatus.WARNING:
//                 return ColorSkin.warningColor
//             case ComponentStatus.ERROR:
//                 return ColorSkin.errorColor
//             case ComponentStatus.SUCCSESS:
//                 return ColorSkin.successColor
//             default:
//                 return ColorSkin.inforColor
//         }
//     }
...
//         switch (this.state.status) {
//             case ComponentStatus.INFOR:
//                 return <SvgXml xml={`<svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
//             style={{ zIndex: 100, backgroundColor: this.getBg(), bottom: this.state.bottom ?? 16 }}
//             visible={this.state.visible === true}
//             duration={2000}
//             onDismiss={() => {
//                 this.setState({ ...this.state, visible: false })
//             }} >
//             {this.state.children ?? <View style={{ flexDirection: 'row', columnGap: 12, alignItems: 'flex-start' }}>
//                 {this.getIcon()}
//                 <View style={{ columnGap: 8, flex: 1 }}>
//                     {this.state.label?.length ? <Text numberOfLines={1} style={[TypoSkin.heading7, { color: '#ffffff' }]}>{this.state.label ?? ''}</Text> : null}
//                     {this.state.message?.length ? <Text numberOfLines={2} style={[TypoSkin.body3, { color: '#ffffff' }]}>{this.state.message ?? ''}</Text> : null}
//                 </View>
//                 {this.state.actionTitle?.length ? <TouchableOpacity onPress={this.state.action}><Text style={[TypoSkin.label4, { color: '#ffffff' }]}>{this.state.actionTitle}</Text></TouchableOpacity> : <View></View>}
//             </View>}
//         </Snackbar>
//     }
// }
...
// export const showSnackbar = ({ message, label, status, children, action, actionTitle, bottom }: {
//     message: string,
//     label?: string,
//     status?: number,
//     action?: () => {},
