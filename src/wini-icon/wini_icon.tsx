...
interface WiniconProps {
  src?: WiniIconName | string; // E.g., "color/accessibility/accessibility"
  id?: string;
  style?: ViewStyle; // Accepts React Native styles
  size?: number; // Size in pixels
  color?: string; // Color to apply
  onClick?: () => void; // Touch event handler
}
    ...
    const fetchSvg = async () => {
      try {
        ...
        if (url) {
          ...
          // Replace stroke and fill colors dynamically
          if (color) {
            text = text.replace(
              /stroke="(?!none")[^"]*"/g,
              `stroke="${color}"`
      ...
      {svgData ? (
        ...
        <View style={[styles.placeholder, { width: size, height: size }]}>
          {/* {imgData ?
                        ...
                        >
                            <SkeletonPlaceholder.Item width={size} height={size} borderRadius={style?.borderRadius ?? 4}></SkeletonPlaceholder.Item>
                        </SkeletonPlaceholder>} */}
        </View>
      )}
    </TouchableOpacity>
...
export type WiniIconName =
  | 'color/accessibility/accessibility-lift'
